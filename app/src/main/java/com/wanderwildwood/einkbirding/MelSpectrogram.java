...
// rather than light-on-dark, and that it is made once from a saved recording rather than
// every 800ms from the live buffer - see BirdSpectrogram.
...
public class MelSpectrogram {
    ...
    /**
     * How far the window moves between frames. Upstream's 960 gives 75 frames for a
     * three-second detection, which stretched across the panel is a smear; it was the
     * right price when this had to be redrawn every 800ms, and the wrong one when it is
     * drawn once. Set by the caller.
     */
    public static int hopLength = HOP_LENGTH;

    /**
     * What fraction of the picture is left white. Upstream stretched from its own minimum
     * to its own maximum; the minimum is the silence floor at -10 while the median of a
     * real detection sits under 3 below the peak, so almost the whole scale was spent on
     * empty bins and everything real came out the same middling grey.
     *
     * A measured recording: peak 4.40, median 1.65, p80 2.23. Which is also why this is a
     * percentile rather than a fixed offset - the spread depends on how close the bird
     * was, and a fixed window that suits a bird in the hedge inks the whole panel for one
     * three fields away.
     */
    public static final float QUIET_FRACTION = 0.80f;

    /**
     * ...but not unboundedly. A recording of nothing at all still has a loudest point, and
     * a pure percentile would dutifully ink the top fifth of the room tone as though it
     * were a bird. Nothing more than this far below the peak gets ink whatever the
     * percentile says, in log10 power, so 3 is 30dB.
     */
    public static final float MAX_RANGE = 3.0f;

    /**
     * Bends the shading towards ink. A straight line from the floor puts most of what is
     * drawn in the pale greys, and pale greys are what this panel is worst at - sixteen
     * levels, dithered, read at arm's length. Below 1 this darkens the middle without
     * touching either end, so quiet stays white and loud stays black.
     */
    ...
    public static Bitmap getMelBitmap(FloatBuffer audioBuffer, int sampleRate, boolean hiRes) {
        return getMelBitmap(audioBuffer, sampleRate, hiRes, HOP_LENGTH);
    }

    public static Bitmap getMelBitmap(FloatBuffer audioBuffer, int sampleRate, boolean hiRes, int hop) {
        hopLength = hop;
    ...
    public static Bitmap calcMelSpectrogramBMP(float[] data) {
        ...
        */
        ...
        // The quietest QUIET_FRACTION of the picture is white; the rest shades to black.
        ...
        float[] sorted = new float[finite];
        int at = 0;
        for (float v : data) {
            if (!Float.isNaN(v) && !Float.isInfinite(v)) sorted[at++] = v;
        }
        Arrays.sort(sorted);
        float min = sorted.length == 0
                ? max - MAX_RANGE
                : sorted[Math.min(sorted.length - 1, (int) (sorted.length * QUIET_FRACTION))];
        if (min < max - MAX_RANGE) min = max - MAX_RANGE;
        float range = max - min;
        if (range <= 0) range = 1;
        ...
        for (int i = 0; i < nFrames; i++) {                 // Time frame
            for (int j = startMel; j < N_MEL; j++) {        // Mel band
                ...
                // Loud is dark. Upstream painted loud as white on black, which is right on
                // a lit screen and wrong on this one: it would put a slab of black on a
                // panel where everything else is black on white, and e-ink holds a large
                // dark field badly.
                int intensity = 255 - (int) (value * 255);
